...
class Sprite:
    ...
    def draw(self, canvas):
        canvas.draw_image(asteroid_image, self.image_center, self.image_size, 
                                self.pos, self.image_size, self.angle)
        canvas.draw_image(missile_image, self.image_center, self.image_size, 
                                self.pos, self.image_size, self.angle)
...
def draw(canvas):
    ...
    # user interface
    canvas.draw_text('Score: ', [WIDTH / 8, HEIGHT / 8], 24, 'white')
    canvas.draw_text(str(score), [WIDTH / 8 + 65, HEIGHT / 8], 24, 'white')
    canvas.draw_text('Lives: ', [WIDTH / 8 * 6 + 35, HEIGHT / 8], 24, 'white')
...
a_missile = Sprite([2 * WIDTH / 3, 2 * HEIGHT / 3], [1,0], 0, 0, missile_image, missile_info, missile_sound)
